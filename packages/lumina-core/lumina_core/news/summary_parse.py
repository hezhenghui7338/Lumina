...
def skim_is_rich(parsed: ParsedRssSummary, *, detail: str | None = None) -> bool:
    """True when RSS already has enough structure for skim without LLM."""
    d = (detail if detail is not None else parsed.detail or "").strip()
    if len(d) >= 80:
        return True
    if len(parsed.viewpoints or []) >= 2:
        return True
    if parsed.quotes:
        return True
    return False
...
def score_hint_from_parsed(parsed: ParsedRssSummary) -> float | None:
    raw = (parsed.meta or {}).get("ai_score") or ""
    try:
