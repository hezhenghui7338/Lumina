...
from lumina_core.config import NEWS_FETCH_MIN_CHARS, NEWS_FETCH_USE_JINA

_USER_AGENT = (
    "Mozilla/5.0 (compatible; Lumina-news/0.1; +https://github.com/lumina)"
)
_HREF_RE = re.compile(
    r"""href\s*=\s*["']([^"']+)["']""",
    re.IGNORECASE,
...
def body_quality_ok(
    ...
    *,
    ...
    body = (text or "").strip()
    floor = min_chars if min_chars is not None else NEWS_FETCH_MIN_CHARS
    if len(body) < floor:
        return False
    ...
    *,
    ...
    for origin in origins[:5]:
        ...
        if (
            ...
            and len(result.text) > max(stub_len * 2, NEWS_FETCH_MIN_CHARS)
        ):
