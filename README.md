...
- **多格式书库** — 电子书 PDF / EPUB / MOBI / AZW / AZW3 / FB2，办公文档 DOCX / ODT / RTF，网页 HTML，以及 Markdown / TXT
- **语义分段** — 识别序言/章节角色，不从句中切断；可手动调界或整书重新分段
- **扫描 PDF OCR** — 默认本机 RapidOCR；可在设置中配置 OpenAI 兼容视觉 API，配置完整时云端优先
- **书架详览** — 智能集合（最近 / 摘要进度 / 分段中 / 导入失败 / 阅读进度 / 分类 / 收藏）+ 网格或列表
- **沉浸式阅读** — 顶栏管书、底栏管段列表 / 笔记 / 深聊 / 显示，摘要 ↔ 原文一键切换
- **听文本** — 听总结 / 听详细 / 听原文，迷你条连播；使用系统离线神经语音
- **书内查找** — **⌘F**（Windows：Ctrl+F）搜该书原文，命中后跳段并高亮
- **深度理解** — 古文、外文自动翻译；段级 citation 可溯源；可选联网增强
- **读书记忆** — 跨书笔记 + **⌘K** 全局搜索，随时找回读过的内容
- **资讯** — RSS 同步、结构化简报与精读深聊
- **永不卡住** — 导入、分段、摘要在后台进行；长书导入时仍可开其他书、资讯与设置
- **可扩展** — 可选 OpenAI / OpenRouter / Cursor 兼容 API；整书摘要导出 Markdown
- **两档摘要** — 每个 API 资源可分别配置正常/高级模型；点「开始摘要」即正常档，点箭头可选高级（需确认）；高级未配置时自动回退正常模型

---

## 界面预览

macOS 主界面（Windows 功能对等，交互为 WinUI 惯用导航）。

| 书库 | 阅读器 |
|:---:|:---:|
| <img src="docs/assets/screenshot-library.jpg" alt="书库：智能集合与书架网格"> | <img src="docs/assets/screenshot-reader.jpg" alt="阅读器：段摘要、听与深聊"> |
| 智能集合 + 网格书架 | 摘要阅读 · 听 · 深聊 |

| 资讯 | 设置 |
|:---:|:---:|
| <img src="docs/assets/screenshot-news.jpg" alt="资讯：今日简报与精读"> | <img src="docs/assets/screenshot-settings.jpg" alt="设置：引擎、听书与 API 资源"> |
| RSS 简报与精读 | 引擎、听书与 API 资源 |
...
- **阅读交互**：段头左右箭头翻段；`↑`/`↓`/`PgUp`/`PgDn` 连续滚动；键盘与触控板路径统一，输入框/弹层避让
- **可配置快捷键（macOS）**：设置可改键、冲突检测、恢复默认；摘要/分段菜单快捷键只打开 popover
- **段顶栏信息**：左上 `章名 · 段 N/总 · 标题`；「原文 · 约 N 字」；总结句墙文质量门
- **工程治理**：版本同步至 1.4.1；badcase catalog 同步

### v1.4 — 性能全方位优化

相对 v1.3 的主要变化：

- **阅读器回退简单路径**：macOS 开书一次拉全量 slim catalog + 主区全书 `ForEach`，去掉有界窗口/滞后锚点带来的视口抖动与骨架死锁风险
- **开书与进度**：Windows 开书先恢复本地进度再 around 拉窗，避免夹到最近段写脏进度；开书贴段顶；慢开书 / 慢段列表打耗时告警
- **Cursor 官方 SDK**：按需下载到用户数据目录，不进 sidecar；设置可装 SDK 并探测连通性
- **摘要质量**：加强总结句冗余/混语碎片门禁；UI 分区「主要内容」仅改可见文案
- **工程治理**：badcase catalog 与 never-freeze-ui / summary-ui-label-only 规则同步；版本同步至 1.4.0
...
- **手动调界**：点相邻两段拼接原文中的某一处预览新分界（吸附句末），点「保存」才落库；取消不保存；不再拖动滑块
...
- **听文本**：阅读器顶栏听总结 / 听详细 / 听原文，迷你条连播与倍速；音色用系统已安装的神经语音（设置里可打开系统下载页），不走云端 TTS
- **书内原文搜索**：顶栏 / ⌘F 只搜该书原文，命中后切到原文模式并高亮；与 ⌘K 跨书搜索分开
- **阅读顶底栏**：点空白同步显隐顶栏（书架、摘要/原文、听、搜索、摘要、分段、导出、导入）与底栏（段列表、深聊、笔记、显示）；正文不位移。五档字号 + 四色纸只涂阅读面
- **永不卡住**：大 TXT / 扫描件的分段放到 sidecar 子进程；导入显示字数或页进度，超时会失败并写明原因；一本分段中仍能打开其他已就绪的书
- **结构分段**：章界硬切、未超限的自然段不劈开；装饰线粘连的章名也能切开；导入可选正常/高级分段
- **摘要更稳**：原文自称「我」时摘要保留第一人称；输出跟目标语言，不串假名/西里尔/英语整句；点「开始摘要」即正常档，点旁边箭头才展开高级
- **书架**：摘要维增加「分段中」「导入失败」；支持无 DRM 的 AZW / AZW3；卡片封面与阅读进度排序
- **引擎**：退出应用会关掉 `lumina-core`；设置始终可查看状态并停止 / 重启
...
- WinUI 惯用 NavigationView / 工具栏 / 抽屉，不复刻 Mac 顶底浮栏
...
| 类型 | 扩展名 |
|------|--------|
| 电子书 | `.pdf` `.epub` `.mobi` `.azw` `.azw3` `.fb2` |
| 办公文档 | `.docx` `.odt` `.rtf` |
| 网页 | `.html` `.htm` `.xhtml` |
...
| 阅读 / 深聊 | 打开书后点空白显隐顶栏与底栏；底栏打开段列表 / 笔记 / 深聊 / 显示（字号与纸色） |
| 听总结 / 听原文 | 阅读器顶栏 **听**；播放中用迷你条暂停、倍速、上下段 |
| 书内原文搜索 | 阅读器顶栏搜索或 **⌘F**（Windows：**Ctrl+F**） |
| 跨书笔记 | 书库 → **全部笔记** |
| 跨书搜索 | **⌘K** |
| 资讯 | 顶部 **资讯** Tab → **同步 RSS** |
| 导出摘要 | 阅读器 → **导出** |
| 任务管理 | **设置** Tab → 查看 / 取消进行中的导入与摘要 |
| 设置 | **设置** Tab（引擎启停、听书音色、语言、联网 provider、深色模式） |
